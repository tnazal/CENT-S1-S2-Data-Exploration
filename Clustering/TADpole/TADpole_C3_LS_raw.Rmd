...
#
# purpose: tadpole clustering on S1 C3 & LS appended, differences, and averages
##################################################################################
```
...
ggplotly(tadpole5_s1_plot) %>% 
  layout(title = "Season 1 with 5 TADpole Clusters (C3 & LS)", 
         margin = list(t = 150))
```
...
ggplotly(tadpole4_s1_plot) %>% 
  layout(title = "Season 1 with 4 TADpole Clusters (C3 & LS)", 
         margin = list(t = 150))
```
...
## TADpole Clustering, 5 Clusters
```{r}
dct_spread <- read.csv("dct_spread_C3_LS_diff.csv")
dct_df <- read.csv("dct_C3_LS_diff.csv")
...
tadpole_clust5_diff <- tsclust(dtw_df_diff, type = "tadpole", k = 5L, 
                               control = tadpole_control(dc = 5, window.size = 25))
...
ggplotly(tadpole5_diff_plot) %>% 
  layout(title = "Season 1 with 5 TADpole Clusters (C3-LS Differences)", 
         margin = list(t = 150))
```
...
## TADpole Clustering, 5 Clusters
```{r}
dct_spread <- read.csv("dct_spread_C3_LS_avg.csv")
dct_df <- read.csv("dct_C3_LS_avg.csv")
...
# diagnostics
diagnostics_tadpole_C3_LS_raw_s1_5cl <- cluster.stats(tadpole_s1_5cl_d, as.numeric(tadpole_clust5_s1_df$cluster))
diagnostics_tadpole_C3_LS_raw_s1_4cl <- cluster.stats(tadpole_s1_4cl_d, as.numeric(tadpole_clust4_s1_df$cluster))
diagnostics_tadpole_C3_LS_raw_diff_s1_5cl <- cluster.stats(tadpole_diff_5cl_d, as.numeric(tadpole_clust5_diff_df$cluster))
diagnostics_tadpole_C3_LS_raw_diff_s1_3cl <- cluster.stats(tadpole_diff_3cl_d, as.numeric(tadpole_clust3_diff_df$cluster))
diagnostics_tadpole_C3_LS_raw_avg_s1_5cl <- cluster.stats(tadpole_avg_5cl_d, as.numeric(tadpole_clust5_avg_df$cluster))
diagnostics_tadpole_C3_LS_raw_avg_s1_3cl <- cluster.stats(tadpole_avg_3cl_d, as.numeric(tadpole_clust3_avg_df$cluster))
...
```{r}
# write table
write.csv(diagnostic_df, "diagnostics_tadpole_C3_LS_raw.csv")
```