---
title: "CENT S1-S2 K-Shape and DTW Clustering on C3 & LS Impressions"
output:
...
#
# purpose: K-Shape and Dynamic Time Warp clustering on C3 & LS appended, 
...
# Clustering Season 1 on C3 & LS Appended
...
ggplotly(clusters_s1_shape5_plot) %>% 
  layout(title = "Season 1 with 5 K-Shape Clusters (C3 & LS)", 
         margin = list(t = 150))
```
...
```{r}
table(dtw_s1$k_4@cluster)
```

```{r}
plot(dtw_s1$k_4, type = 'sc')
```

```{r}
# export cluster predictions to the main data set
clusters_s1_dtw4_spread <- dct_spread %>% filter(Season_num == 1)
clusters_s1_dtw4_spread$cluster_dtw_4 <- dtw_s1$k_4@cluster

clusters_s1_dtw4 <- merge(clusters_s1_dtw4_spread[, c("cluster_dtw_4", "Show_Name")], 
                          dct_df, by = "Show_Name")
```

```{r}
clusters_s1_dtw4_plot <- clusters_s1_dtw4 %>%
  filter(Season_num == 1) %>% 
  ggplot(aes(x = period, y = dct_values, color = Show_Name)) + 
  geom_line() + 
...
ggplotly(clusters_s1_dtw4_plot) %>% 
  layout(title = "Season 1 with 4 DTW Clusters (C3 & LS)", 
         margin = list(t = 150))
```


# S1 Clustering on C3-LS Differencs 

```{r}
dct_diff <- read.csv("dct_C3_LS_diff.csv")
dct_spread_diff <- read.csv("dct_spread_C3_LS_diff.csv")
```
...
```{r}
table(kshape_diff$k_4@cluster)
```

```{r}
plot(kshape_diff$k_4, type = 'sc')
```
...
```{r}
clusters_diff_shape4_plot <- clusters_diff_shape4 %>%
  ggplot(aes(x = period, y = dct_values, color = Show_Name)) + 
  geom_line() + 
...
ggplotly(clusters_diff_shape4_plot) %>% 
  layout(title = "S1 with 4 K-Shape Clusters (C3-LS Differences)", 
         margin = list(t = 150))
```
...
```{r}
table(dtw_diff$k_3@cluster)
```

```{r}
plot(dtw_diff$k_3, type = 'sc')
```

```{r}
# export cluster predictions to the main data set
clusters_diff_dtw3_spread <- dct_spread_diff %>% filter(Season_num == 1)
clusters_diff_dtw3_spread$cluster_dtw_3 <- dtw_diff$k_3@cluster

clusters_diff_dtw3 <- merge(clusters_diff_dtw3_spread[, c("cluster_dtw_3", "Show_Name")], 
                             dct_diff, by = "Show_Name")
```

```{r}
clusters_diff_dtw3_plot <- clusters_diff_dtw3 %>% 
  ggplot(aes(x = period, y = dct_values, color = Show_Name)) + 
  geom_line() + 
...
ggplotly(clusters_diff_dtw3_plot) %>% 
  layout(title = "S1 with 5 DTW Clusters (C3-LS Differences)", 
         margin = list(t = 150))
```


# Season 1 Clustering on C3 & LS Averages

```{r}
dct_avg <- read.csv("dct_C3_LS_avg.csv")
dct_spread_avg <- read.csv("dct_spread_C3_LS_avg.csv")
...
```{r}
kshape_avg <- tsclust(cluster_diff, 
                       type = 'partitional',
                       k = 2:5,
                       preproc = zscore,
                       distance = 'sbd',
...
```{r}
table(kshape_avg$k_4@cluster)
```

```{r}
plot(kshape_avg$k_4, type = 'sc')
```

```{r}
# export cluster predictions to the main data set
clusters_avg_shape4_spread <- dct_spread_avg %>% filter(Season_num == 1)
clusters_avg_shape4_spread$cluster_shape_4 <- kshape_avg$k_4@cluster

clusters_avg_shape4 <- merge(clusters_avg_shape4_spread[, c("cluster_shape_4", "Show_Name")], 
                              dct_avg, by = "Show_Name")
```

```{r}
clusters_avg_shape4_plot <- clusters_avg_shape4 %>%
  ggplot(aes(x = period, y = dct_values, color = Show_Name)) + 
  geom_line() + 
...
ggplotly(clusters_avg_shape4_plot) %>% 
  layout(title = "S1 with 4 K-Shape Clusters (C3 & LS Averages)", 
         margin = list(t = 150))
```
...
```{r}
table(dtw_avg$k_2@cluster)
```

```{r}
plot(dtw_avg$k_2, type = 'sc')
```

```{r}
# export cluster predictions to the main data set
clusters_avg_dtw2_spread <- dct_spread_avg
clusters_avg_dtw2_spread$cluster_dtw_2 <- dtw_avg$k_2@cluster

clusters_avg_dtw2 <- merge(clusters_avg_dtw2_spread[, c("cluster_dtw_2", "Show_Name")], 
                            dct_avg, by = "Show_Name")

clusters_avg_dtw2_spread <- clusters_avg_dtw2_spread %>% filter(Season_num == 1)
```

```{r}
clusters_avg_dtw2_plot <- clusters_avg_dtw2 %>% 
  ggplot(aes(x = period, y = dct_values, color = Show_Name)) + 
  geom_line() + 
...
ggplotly(clusters_avg_dtw2_plot) %>% 
  layout(title = "S1 with 2 DTW Clusters (C3 & LS Averages)", 
         margin = list(t = 150))
```
...
s1_4clusters_d <- dist(clusters_s1_dtw4_spread[, 5:205])
diff_4clusters_d <- dist(clusters_diff_shape4_spread[, 5:105])
diff_3clusters_d <- dist(clusters_diff_dtw3_spread[, 5:105])
avg_4clusters_d <- dist(clusters_avg_shape4_spread[, 5:105])
avg_2clusters_d <- dist(clusters_avg_dtw2_spread[, 5:105])

# diagnostics
diagnostics_kshape_C3_LS_raw_s1_5cl <- cluster.stats(s1_5clusters_d, as.numeric(clusters_s1_shape5_spread$cluster_shape_5))
diagnostics_dtw_C3_LS_raw_s1_4cl <- cluster.stats(s1_4clusters_d, as.numeric(clusters_s1_dtw4_spread$cluster_dtw_4))
diagnostics_kshape_C3_LS_raw_diff_s1_4cl <- cluster.stats(diff_4clusters_d, as.numeric(clusters_diff_shape4_spread$cluster_shape_4))
diagnostics_dtw_C3_LS_raw_diff_s1_3cl <- cluster.stats(diff_3clusters_d, as.numeric(clusters_diff_dtw3_spread$cluster_dtw_3))
diagnostics_kshape_C3_LS_raw_avg_s1_4cl <- cluster.stats(avg_4clusters_d, as.numeric(clusters_avg_shape4_spread$cluster_shape_4))
diagnostics_dtw_C3_LS_raw_avg_s1_2cl <- cluster.stats(avg_2clusters_d, as.numeric(clusters_avg_dtw2_spread$cluster_dtw_2))
...
```{r}
# write table
write.csv(diagnostic_df, "diagnostics_kshape_dtw_C3_LS_raw.csv")
```