---
title: "CENT S1-S2 K-Shape and DTW Clustering on LS Impressions"
output:
...
```{r}
##################################################################################
# inputs: dct_LS_raw.csv, dct_LS_raw_cont.csv, dct_LS_raw_s1_s2.csv,
#         dct_spread_LS_raw.csv, dct_spread_LS_raw_cont.csv 
#
...
```{r}
dct_df <- read.csv("dct_LS_raw.csv")
dct_spread <- read.csv("dct_spread_LS_raw.csv")
```
...
clusters_s1_shape2_plot <- clusters_s1_shape2 %>%
  ...
  geom_line() + 
  facet_wrap(~ cluster_shape_2) +  
  theme(axis.title.x = element_blank(), 
        axis.title.y = element_blank())

ggplotly(clusters_s1_shape2_plot) %>% 
  layout(title = "Season 1 with 2 K-Shape Clusters (LS)", 
         margin = list(t = 150))
```
...
ggplotly(s1_2clusters_kshape_s2_trends_plot) %>% 
  layout(title = "Season 1 with 2 K-Shape Clusters & Season 2 Trends (LS)", 
         margin = list(t = 150))
```
...
write.csv(clusters_s1_dtw3, "LS_raw_s1_3dtw_clusters.csv")
```
...
ggplotly(clusters_s1_dtw3_plot) %>% 
  layout(title = "Season 1 with 3 DTW Clusters (LS)", 
         margin = list(t = 150))
```
...
write.csv(dct_s1_3clusters_dtw_s2_trends, "LS_raw_s1_3dtw_s2trends.csv")
```
...
ggplotly(s1_3clusters_dtw_s2_trends_plot) %>% 
  layout(title = "Season 1 with 3 DTW Clusters & Season 2 Trends (LS)", 
         margin = list(t = 150))
```

```{r}
# TEST PLOT FOR S1 CLUSTERS & s2 TRENDS BY CLUSTER
dct_s1_3clusters_dtw_s2_trends_plot <- plot_ly(dct_s1_3clusters_dtw_s2_trends, 
        x = ~period, 
        y = ~dct_values, 
        color = ~Show_Name,
...
dct_s1_3clusters_dtw_s2_trends_plot
```
...
```{r}
table(kshape_s2$k_5@cluster)
```

```{r}
plot(kshape_s2$k_5, type = 'sc')
```

```{r}
# export cluster predictions to the main data set
clusters_s2_shape5_spread <- dct_spread %>% filter(Season_num == 2)
clusters_s2_shape5_spread$cluster_shape_5 <- kshape_s2$k_5@cluster

clusters_s2_shape5 <- merge(clusters_s2_shape5_spread[, c("cluster_shape_5", "Show_Name")], 
                            dct_df, by = "Show_Name")
```

```{r}
clusters_s2_shape5_plot <- clusters_s2_shape5 %>%
  filter(Season_num == 2) %>% 
  ggplot(aes(x = period, y = dct_values, color = Show_Name)) + 
  geom_line() + 
...
ggplotly(clusters_s2_shape5_plot) %>% 
  layout(title = "Season 2 with 5 K-Shape Clusters (LS)", 
         margin = list(t = 150))
```

```{r}
s2_5clusters_kshape <- list(as.character(unique(clusters_s2_shape5[clusters_s2_shape5$cluster_shape_5 == 1,]$Show_Name)), 
                            as.character(unique(clusters_s2_shape5[clusters_s2_shape5$cluster_shape_5 == 2,]$Show_Name)), 
                            as.character(unique(clusters_s2_shape5[clusters_s2_shape5$cluster_shape_5 == 3,]$Show_Name)), 
                            as.character(unique(clusters_s2_shape5[clusters_s2_shape5$cluster_shape_5 == 4,]$Show_Name)), 
                            as.character(unique(clusters_s2_shape5[clusters_s2_shape5$cluster_shape_5 == 5,]$Show_Name)))
s2_5clusters_kshape
```
...
```{r}
table(dtw_s2$k_3@cluster)
```

```{r}
plot(dtw_s2$k_3, type = 'sc')
```

```{r}
# export cluster predictions to the main data set
clusters_s2_dtw3_spread <- dct_spread %>% filter(Season_num == 2)
clusters_s2_dtw3_spread$cluster_dtw_3 <- dtw_s2$k_3@cluster

clusters_s2_dtw3 <- merge(clusters_s2_dtw3_spread[, c("cluster_dtw_3", "Show_Name")], 
                          dct_df, by = "Show_Name")
```

```{r}
clusters_s2_dtw3_plot <- clusters_s2_dtw3 %>%
  filter(Season_num == 2) %>% 
  ggplot(aes(x = period, y = dct_values, color = Show_Name)) + 
  geom_line() + 
  facet_wrap(~ cluster_dtw_3, nrow = 3) +  
...
ggplotly(clusters_s2_dtw3_plot) %>% 
  layout(title = "Season 2 with 3 DTW Clusters (LS)", 
         margin = list(t = 150))
```

```{r}
s2_3clusters_dtw <- list(as.character(unique(clusters_s2_dtw3[clusters_s2_dtw3$cluster_dtw_3 == 1,]$Show_Name)), 
                         as.character(unique(clusters_s2_dtw3[clusters_s2_dtw3$cluster_dtw_3 == 2,]$Show_Name)), 
                         as.character(unique(clusters_s2_dtw3[clusters_s2_dtw3$cluster_dtw_3 == 3,]$Show_Name)))
s2_3clusters_dtw
```
...
dct_s1_s2 <- read.csv("dct_LS_raw_s1_s2.csv")
```
...
ggplotly(clusters_s1_s2_shape5_plot) %>% 
  layout(title = "S1-S2 (Non-Continuous) with 5 K-Shape Clusters (LS)", 
         margin = list(t = 150))
```
...
```{r}
table(dtw_s1_s2$k_4@cluster)
```

```{r}
plot(dtw_s1_s2$k_4, type = 'sc')
```

```{r}
# export cluster predictions to the main data set
clusters_s1_s2_dtw4_spread <- dct_spread_s1_s2
clusters_s1_s2_dtw4_spread$cluster_dtw_4 <- dtw_s1_s2$k_4@cluster

clusters_s1_s2_dtw4 <- merge(clusters_s1_s2_dtw4_spread[, c("cluster_dtw_4", "Show_Name")], 
                             dct_s1_s2, by = "Show_Name")
```

```{r}
clusters_s1_s2_dtw4_plot <- clusters_s1_s2_dtw4 %>% 
  ggplot(aes(x = period, y = dct_values, color = Show_Name)) + 
  geom_line() + 
...
ggplotly(clusters_s1_s2_dtw4_plot) %>% 
  layout(title = "S1-S2 (Non-Continuous) with 4 DTW Clusters (LS)", 
         margin = list(t = 150))
```

```{r}
s1_s2_4clusters_dtw <- list(as.character(unique(clusters_s1_s2_dtw4[clusters_s1_s2_dtw4$cluster_dtw_4 == 1,]$Show_Name)), 
                            as.character(unique(clusters_s1_s2_dtw4[clusters_s1_s2_dtw4$cluster_dtw_4 == 2,]$Show_Name)), 
                            as.character(unique(clusters_s1_s2_dtw4[clusters_s1_s2_dtw4$cluster_dtw_4 == 3,]$Show_Name)), 
                            as.character(unique(clusters_s1_s2_dtw4[clusters_s1_s2_dtw4$cluster_dtw_4 == 4,]$Show_Name)))
s1_s2_4clusters_dtw
```
...
```{r}
dct_cont <- read.csv("dct_LS_raw_cont.csv")
dct_spread_cont <- read.csv("dct_spread_LS_raw_cont.csv")
...
ggplotly(clusters_cont_shape4_plot) %>% 
  layout(title = "S1-S2 (Continuous) with 4 K-Shape Clusters (LS)", 
         margin = list(t = 150))
```
...
ggplotly(clusters_cont_dtw2_plot) %>% 
  layout(title = "S1-S2 (Continuous) with 2 DTW Clusters (LS)", 
         margin = list(t = 150))
```
...
```{r}
# S1-S2 Trends Clustered on S1 3 DTW Clusters
season1_dtw3_clusters <- clusters_s1_dtw3_spread %>% 
  select(Show_Name, cluster_dtw_3) %>% 
  rename(s1_cluster = cluster_dtw_3)

s1_3clusters_cont_trends <- merge(season1_dtw3_clusters, 
                                  dct_spread_cont, by = "Show_Name") %>% 
  select(-X)

s1_3clusters_cont_trends <- merge(s1_3clusters_cont_trends[, c("Show_Name", "s1_cluster")], 
                                  dct_cont, by = "Show_Name") %>% 
  select(-X)

write.csv(s1_3clusters_cont_trends, "LS_raw_s1_3dtw_cont_trends.csv") 
```

```{r}
# Plot for Continuous Trends Clustered on S1 Clusters
s1_3clusters_cont_trends_plot <- plot_ly(s1_3clusters_cont_trends, 
        x = ~period, 
        y = ~dct_values, 
        color = ~Show_Name,
  ...
  layout(showlegend = TRUE, 
         title = "S1-S2 Clustered on S1 Clusters (LS)", 
         autosize = FALSE, width = 650, height = 750, 
         margin = list(l = 50, 
                       r = 50, 
                       b = 50, 
                       t = 125))

s1_3clusters_cont_trends_plot
```
...
s2_5clusters_d <- dist(clusters_s2_shape5_spread[, 5:105])
s1_3clusters_d <- dist(clusters_s1_dtw3_spread[, 5:105])
s2_3clusters_d <- dist(clusters_s2_dtw3_spread[, 5:105])
...
s1_s2_4clusters_d <- dist(clusters_s1_s2_dtw4_spread[, 5:204])

# diagnostics
diagnostics_kshape_LS_raw_s1_2cl <- cluster.stats(s1_2clusters_d, as.numeric(clusters_s1_shape2_spread$cluster_shape_2))
diagnostics_kshape_LS_raw_s2_5cl <- cluster.stats(s2_5clusters_d, as.numeric(clusters_s2_shape5_spread$cluster_shape_5))
diagnostics_kshape_LS_raw_cont_4cl <- cluster.stats(cont_4clusters_d, as.numeric(clusters_cont_shape4_spread$cluster_shape_4))
diagnostics_kshape_LS_raw_sep_5cl <- cluster.stats(s1_s2_5clusters_d, as.numeric(clusters_s1_s2_shape5_spread$cluster_shape_5))
diagnostics_dtw_LS_raw_s1_3cl <- cluster.stats(s1_3clusters_d, as.numeric(clusters_s1_dtw3_spread$cluster_dtw_3))
diagnostics_dtw_LS_raw_s2_3cl <- cluster.stats(s2_3clusters_d, as.numeric(clusters_s2_dtw3_spread$cluster_dtw_3))
diagnostics_dtw_LS_raw_cont_2cl <- cluster.stats(cont_2clustdtw_d, as.numeric(clusters_cont_dtw2_spread$cluster_dtw_2))
diagnostics_dtw_LS_raw_sep_4cl <- cluster.stats(s1_s2_4clusters_d, as.numeric(clusters_s1_s2_dtw4_spread$cluster_dtw_4))
...
```{r}
# write table
write.csv(diagnostic_df, "diagnostics_kshape_dtw_LS_raw.csv")
```