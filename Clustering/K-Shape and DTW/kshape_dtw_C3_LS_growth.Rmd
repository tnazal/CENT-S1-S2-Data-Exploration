...
```{r}
table(dtw_s1$k_2@cluster)
```

```{r}
plot(dtw_s1$k_2, type = 'sc')
```
...
```{r}
table(kshape_diff$k_2@cluster)
```

```{r}
plot(kshape_diff$k_2, type = 'sc')
```

```{r}
# export cluster predictions to the main data set
clusters_diff_shape2_spread <- dct_spread_diff %>% filter(Season_num == 1)
clusters_diff_shape2_spread$cluster_shape_2 <- kshape_diff$k_2@cluster

clusters_diff_shape2 <- merge(clusters_diff_shape2_spread[, c("cluster_shape_2", "Show_Name")], 
                               dct_diff, by = "Show_Name")
```

```{r}
clusters_diff_shape2_plot <- clusters_diff_shape2 %>%
  ggplot(aes(x = period, y = dct_values, color = Show_Name)) + 
  geom_line() + 
...
ggplotly(clusters_diff_shape2_plot) %>% 
  layout(title = "S1 with 2 K-Shape Clusters (C3-LS Growth Differences)", 
         margin = list(t = 150))
```
...
```{r}
table(dtw_diff$k_4@cluster)
```

```{r}
plot(dtw_diff$k_4, type = 'sc')
```
...
```{r}
table(kshape_avg$k_5@cluster)
```

```{r}
plot(kshape_avg$k_5, type = 'sc')
```
...
```{r}
table(dtw_avg$k_3@cluster)
```

```{r}
plot(dtw_avg$k_3, type = 'sc')
```
...
diff_2clusters_d <- dist(clusters_diff_shape2_spread[, 5:105])
diff_4clusters_d <- dist(clusters_diff_dtw4_spread[, 5:105])
...
diagnostics_kshape_C3_LS_growth_diff_s1_2cl <- cluster.stats(diff_2clusters_d, as.numeric(clusters_diff_shape2_spread$cluster_shape_2))
diagnostics_dtw_C3_LS_growth_diff_s1_4cl <- cluster.stats(diff_4clusters_d, as.numeric(clusters_diff_dtw4_spread$cluster_dtw_4))
