---
title: "CENT S1-S2 K-Means Clustering on LS Impressions"
output:
...
```{r}
########################################################################
# inputs: dct_LS_raw.csv, dct_spread_LS_raw.csv, dct_LS_raw_s1_s2.csv,
#         dct_LS_raw_cont.csv, dct_spread_LS_raw_cont.csv 
#
...
ggplotly(clusters3_s1_plot) %>% 
  layout(title = "Season 1 with 3 K-Means Clusters (LS)", 
         margin = list(t = 150))
```
...
ggplotly(s1_3clusters_s2_trends_plot) %>% 
  layout(title = "Season 1 with 3 K-Means Clusters & Season 2 Trends (LS)", 
         margin = list(t = 150))
```
...
ggplotly(clusters4_s1_plot) %>% 
  layout(title = "Season 1 with 4 K-Means Clusters (LS)", 
         margin = list(t = 150))
```
...
ggplotly(s1_4clusters_s2_trends_plot) %>% 
  layout(title = "Season 1 with 4 K-Means Clusters & Season 2 Trends (LS)", 
         margin = list(t = 150))
```
...
write.csv(clusters5_s1, "kmeans_LS_raw_s1_5cl.csv")
```
...
ggplotly(clusters5_s1_plot) %>% 
  layout(title = "Season 1 with 5 K-Means Clusters (LS)", 
         margin = list(t = 150))
```
...
write.csv(dct_s1_5clusters_s2_trends, "kmeans_LS_raw_s1_5cl_s2trends.csv")
```
...
ggplotly(s1_5clusters_s2_trends_plot) %>% 
  layout(title = "Season 1 with 5 K-Means Clusters & Season 2 Trends (LS)", 
         margin = list(t = 150))
```

```{r}
# PLOT WITH GROUPINGS BY NETWORK
plot_ly(dct_s1_5clusters_s2_trends, 
  ...
  layout(showlegend = TRUE, 
         title = "Season 1 with 5 K-Means Clusters & Season 2 Trends (LS)")
```

```{r}
# PLOT WITH GROUPINGS BY CLUSTER
plot_ly(dct_s1_5clusters_s2_trends, 
        x = ~period, 
        y = ~dct_values, 
        color = ~Show_Name,
...
```{r}
#same as clustering for season 1 but with season 2 subsets
...
ggplotly(clusters3_s2_plot) %>% 
  layout(title = "Season 2 with 3 K-Means Clusters (LS)", 
         margin = list(t = 150))
```
...
ggplotly(clusters4_s2_plot) %>% 
  layout(title = "Season 2 Shows with 4 Clusters(LS)", 
         margin = list(t = 150))
```
...
ggplotly(clusters5_s2_plot) %>% 
  layout(title = "Season 2 with 5 K-Means Clusters (LS)", 
         margin = list(t = 150))
```
...
ggplotly(clusters3_s1_s2_plot) %>% 
  layout(title = "S1-S2 (Non-Continuous) with 3 K-Means Clusters (LS)", 
         margin = list(t = 150))
```
...
ggplotly(clusters4_s1_s2_plot) %>% 
  layout(title = "S1-S2 (Non-Continuous) with 4 K-Means Clusters (LS)", 
         margin = list(t = 150))
```
...
ggplotly(clusters5_s1_s2_plot) %>% 
  layout(title = "S1-S2 (Non-Continuous) with 5 K-Means Clusters (LS)", 
         margin = list(t = 150))
```
...
ggplotly(clusters3_cont_plot) %>% 
  layout(title = "S1-S2 (Continuous) with 3 K-Means Clusters (LS)", 
         margin = list(t = 150))
```
...
ggplotly(clusters5_cont_plot) %>% 
  layout(title = "S1-S2 (Continuous) with 5 K-Means Clusters (LS)", 
         margin = list(t = 150))
```
...
season1_clusters <- clusters5_s1_spread %>% 
  select(Show_Name, cluster_kmeans_5) %>% 
  rename(s1_cluster = cluster_kmeans_5)

s1_clusters_cont_trends <- merge(season1_clusters, 
                                 dct_spread_cont, by = "Show_Name") %>% select(-X)

s1_clusters_cont_trends <- merge(s1_clusters_cont_trends[, c("Show_Name", "s1_cluster")], 
                                 dct_cont, by = "Show_Name") %>% select(-X)

write.csv(s1_clusters_cont_trends, "kmeans_LS_raw_s1_5cl_cont_trends.csv") 
```
...
kmeans_s1_s2_4cl_d <- dist(clusters4_s1_s2_spread[, 6:205])
...
# diagnostics
diagnostics_kmeans_LS_raw_s1_3cl <- cluster.stats(kmeans_s1_3cl_d, as.numeric(clusters3_s1_spread$cluster_kmeans_3))
diagnostics_kmeans_LS_raw_s1_4cl <- cluster.stats(kmeans_s1_4cl_d, as.numeric(clusters4_s1_spread$cluster_kmeans_4))
diagnostics_kmeans_LS_raw_s1_5cl <- cluster.stats(kmeans_s1_5cl_d, as.numeric(clusters5_s1_spread$cluster_kmeans_5))
diagnostics_kmeans_LS_raw_s2_3cl <- cluster.stats(kmeans_s2_3cl_d, as.numeric(clusters3_s2_spread$cluster_kmeans_3))
diagnostics_kmeans_LS_raw_s2_4cl <- cluster.stats(kmeans_s2_4cl_d, as.numeric(clusters4_s2_spread$cluster_kmeans_4))
diagnostics_kmeans_LS_raw_s2_5cl <- cluster.stats(kmeans_s2_5cl_d, as.numeric(clusters5_s2_spread$cluster_kmeans_5))
diagnostics_kmeans_LS_raw_sep_3cl <- cluster.stats(kmeans_s1_s2_3cl_d, as.numeric(clusters3_s1_s2_spread$cluster_kmeans_3))
diagnostics_kmeans_LS_raw_sep_4cl <- cluster.stats(kmeans_s1_s2_4cl_d, as.numeric(clusters4_s1_s2_spread$cluster_kmeans_4))
diagnostics_kmeans_LS_raw_sep_5cl <- cluster.stats(kmeans_s1_s2_5cl_d, as.numeric(clusters5_s1_s2_spread$cluster_kmeans_5))
diagnostics_kmeans_LS_raw_cont_3cl <- cluster.stats(kmeans_cont_3cl_d, as.numeric(clusters3_cont_spread$cluster_kmeans_3))
diagnostics_kmeans_LS_raw_cont_5cl <- cluster.stats(kmeans_cont_5cl_d, as.numeric(clusters5_cont_spread$cluster_kmeans_5))
...
```{r}
# write table
write.csv(diagnostic_df, "diagnostics_kmeans_LS_raw.csv")
```
