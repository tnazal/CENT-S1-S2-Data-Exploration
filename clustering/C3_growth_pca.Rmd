---
title: "CENT S1-S2 PCA Clustering on C3 Growth"
output:
...
```{r}
##################################################################################
# inputs: CENT_growth_dct_spread.csv, CENT_dct_spread_cont_growth.csv, 
#         CENT_growth_dct.csv
#
...
ggplotly(pca_s1_plot) %>% 
  layout(title = "Season 1 with 3 PCA Clusters (C3 Growth)", 
         margin = list(t = 150))
```
...
ggplotly(s1_pcaclusters_s2_trends_plot) %>% 
  layout(title = "Season 1 with 3 PCA Clusters & Season 2 Trends (C3 Growth)", 
         margin = list(t = 150))
```
...
ggplotly(pca_s2_plot) %>% 
  layout(title = "Season 2 with 3 PCA Clusters (C3 Growth)", 
         margin = list(t = 150))
```
...
ggplotly(pca_cont_plot) %>% 
  layout(title = "S1-S2 (Continuous) with 3 PCA Clusters (C3 Growth)", 
         margin = list(t = 150))
```
...
```{r}
pca_cont_clusters_growth <- list(as.character(unique(df_pcaclusters_cont[df_pcaclusters_cont$cluster == 1,]$Show_Name)),
...
ggplotly(pca_s1_s2_plot) %>% 
  layout(title = "S1-S2 (Non-Continuous) with 4 PCA Clusters (C3 Growth)", 
         margin = list(t = 150))
```
...
```{r}
pca_s1_s2_clusters_growth <- list(as.character(unique(df_pcaclusters_s1_s2[df_pcaclusters_s1_s2$cluster == 1,]$Show_Name)),
...
# diagnostics
diagnostics_C3_growth_pca_s1_3cl <- cluster.stats(pca_s1_3cl_d , as.numeric(df_pcaclusters_s1$cluster))
diagnostics_C3_growth_pca_s2_3cl <- cluster.stats(pca_s2_3cl_d , as.numeric(df_pcaclusters_s2$cluster))
diagnostics_C3_growth_pca_cont_3cl <- cluster.stats(pca_cont_3cl_d , as.numeric(df_pcaclusters_cont$cluster))
diagnostics_C3_growth_pca_sep_4cl <- cluster.stats(pca_sep_4cl_d , as.numeric(df_pcaclusters_s1_s2$cluster))
...
```{r}
# write table
write.csv(diagnostic_df, "diagnostics_C3_growth_pca.csv")
```
